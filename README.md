### ЗАПУСК ТЕСТИРОАНИЯ
- Для запуска юнит тестов и функциональных тестов {pytest tests/ -v}
- Для запуска нагрузочного тестирования {locust -f locustfile.py --host=http://localhost:8000}
- Для генерации отчета
  1. coverage run -m pytest tests (запуск тестов)
  2. coverage report (для отчета в консоли)
  3. coverage html (для генерации файла отчета)

- В папке "ОТЧЕТ НАГРУЗКИ" представлен файл по нагрузочному тестированию
- В папке "ОТЧЕТ ФУНКЦИОНАЛЬНЫХ" представлен отчет по тестам
